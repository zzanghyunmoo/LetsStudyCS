...
4. **창작**: 같은 개념으로 자신만의 작은 결과를 만듭니다. 게임 문제는 2장부터 시작합니다.

단계가 진행될수록 안내와 힌트는 줄어들지만, 테스트는 끝까지 안전망으로 남습니다.

## 저장소가 자라는 방식

현재 1장은 다음 구조로 완성되어 있습니다.
...
- `LetsStudyCS.slnx`: 완성 예제와 해답을 한 번에 빌드하고 테스트하는 루트 솔루션

게임은 2장부터 `game/`에 하나만 만들 예정입니다. 연습과 해답은 장별로 격리해 복구하기 쉽게 만들고, 실제 게임은 이전 장의 결과를 다음 장으로 이어갑니다.

## 1장: C#과 첫 프로그램

1장은 게임을 만들기 전에 출발점을 맞추는 온보딩 장입니다.

바로 시작하려면 [1장 학습 가이드](chapters/01-getting-started/README.md)를 여세요.
...
연습용 테스트는 처음에는 의도적으로 실패합니다. 실패를 확인하고 통과시키는 정확한 순서는 1장 학습 가이드에 있습니다.
...
1장의 예제·연습·해답·xUnit 테스트가 준비되었습니다. 1장 완료 기준을 모두 확인한 뒤에만 2장의 게임 보드 출력을 설계합니다.
